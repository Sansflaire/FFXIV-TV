...
### NVIDIA Client Compatibility — Format Fix + Deep Diag (v0.5.172)
- [x] Add R10G10B10A2_UNorm to IsLdrFullRes whitelist (NVIDIA HDR/RTX HDR/Smooth Motion)
- [x] Expand /fftv diag line 2: vp.M11 (ViewProj validity), last inject fmt+size, screen center, cfdi/dispatch/ldr counts

### NVIDIA Client Compatibility — Diag + Fallback (v0.5.171)
- [x] Add `/fftv diag` command — prints D3D inject state to chat (D3D init, target ptr, inject count, path, bb/dsv learned, frames)
- [x] Add ImGui fallback: if D3D inject fires 0 times after 180 frames (~3s), fall back to Phase 1 overlay
- [x] Fallback makes screen visible on any GPU where inject pipeline doesn't match; logs warning once

### Debug/Settings Cleanup (v0.5.170)
- [x] Remove Debug tab from MainWindow entirely (gradient sliders, diagnostics, shader param dupes)
- [x] Remove inject toggle checkboxes from Settings tab (CF-DI, CF-Draw, OMSetRT-LDR, etc.)
- [x] Remove Phase 1 Sandbox checkbox from Settings tab and `UsePhase1Sandbox` from Configuration
- [x] Remove `UsePhase1Sandbox` sandbox branch from Plugin.OnDraw and StatusApi endpoints
- [x] All settings that affected rendering behavior are now hardcoded to their correct defaults

### Client Compatibility Fix (v0.5.169)
- [x] Fix: `_targetInjectRtvPtr` learned wrong BGRA8 surface on other GPU/settings configs
- [x] On client: inject was hitting a scene intermediate → video showed on characters, not screen
- [x] Fix: when BB binds, `prevNoDsvPtr` (surface bound just before BB) = definitive LDR target
- [x] Force-override `_lastSeenValidRtvPtr` at BB-bind time for reliable cross-machine learning

### Cloudflared Tunnel (v0.5.168)
- [x] "Create Room" button spawns cloudflared subprocess (auto-downloads binary on first use)
- [x] Tunnel URL (`wss://xxx.trycloudflare.com`) shown in UI — click to copy
- [x] "LAN Only" button kept for same-network direct TCP (no cloudflared needed)
- [x] `SyncClient` normalizes `*.trycloudflare.com` addresses to `wss://` URI automatically
- [x] `BuildFileUrl()` prefers tunnel URL for local file serving (converts wss:// → https://)
- [x] State-on-connect: screen config + playback state broadcast to each client on join
- [x] ✅ CONFIRMED WORKING: client connects to host via tunnel URL, syncs screen + playback
...
## Phase 4.5 — QoL Commands

- [x] `/fftv version` — posts plugin version to echo chat
...
- [x] Fix: HUD renders in front of rect (v0.5.163/164) — CF-DI inject-first on `_targetInjectRtvPtr` + depth testing; see Golden-Standard-Rendering.md
- [x] v0.5.57/v0.5.58: Scene-pass inject — attempted; fired too early (before FFXIV clears+draws scene); v0.5.57 blocked BB fallback causing invisible rect; v0.5.58 restored BB fallback
...
- [x] v0.5.117: Two-pass depth-only scene inject + LDR OMSetRT inject — video visible (correct colors ✓), HUD behind rect (OMSetRT inject fired into surface already containing HUD) ✗
- [x] v0.5.118: Removed OMSetRT LDR inject; enabled DrawIndexed BB inject for all cases; Draw BB inject still present (wrong: fires on pre-composite HUD Draw) → HUD still behind
- [x] v0.5.119: Removed Draw BB inject entirely; only DrawIndexed BB inject remains — BUT ldrInjectCount=0 (no color inject firing); video not rendered to screen at all
- [x] v0.5.120: Diagnostic instrumentation — logs DI/Draw condition values for first 5 frames to identify why ldrInjectCount=0
- [x] v0.5.158: Renderer singleton + GetLiveBackbufferRtv() — resolve FFXIV's renderer global ptr via SigScanner at init; walk renderer→+0x70→+0x60→+0x68 to get live BB RTV each frame; IsBackbuffer() helper replaces all _knownBackbufferRtvPtrs.Contains() calls; _cachedLiveRtv cached per-frame in PrepareHooks; fixes ldrInjectCount=0 caused by fragile learned swapchain buffer rotation
...
- **~~Game native 2D UI renders BEHIND the video screen~~ — FIXED in v0.5.163.** All HUD (chat, hotbar, map) now renders in front of the TV rect. Depth testing also correct (characters occlude TV). See BROKEN.md for the full history of what was tried.
