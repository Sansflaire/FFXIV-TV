...
public sealed class VideoPlayer : IDisposable
{
    ...
    /// <summary>Current decoded frame texture dimensions (0×0 when no texture).</summary>
    public (int W, int H) TextureSize => (_texWidth, _texHeight);

    /// <summary>
    /// Samples a single pixel from the CPU-side decoded frame buffer at the given UV (0–1).
    /// Returns (B, G, R, A) in BGRA byte order. Safe to call from any thread — may read a
    /// partially-written frame under VLC decode, but that is acceptable for diagnostics.
    /// Returns (0,0,0,0) when no frame is available.
    /// </summary>
    public (byte B, byte G, byte R, byte A) SamplePixelBgra(float u, float v)
    {
        var px = _pixels;
        if (px == null || _pixelWidth <= 0 || _pixelHeight <= 0) return (0, 0, 0, 0);
        int x   = Math.Clamp((int)(u * _pixelWidth),  0, _pixelWidth  - 1);
        int y   = Math.Clamp((int)(v * _pixelHeight), 0, _pixelHeight - 1);
        int idx = (y * _pixelWidth + x) * 4;
        if (idx + 3 >= px.Length) return (0, 0, 0, 0);
    ...
    private void EnsureVlcInitialized()
    {
        ...
        // Disable hardware video acceleration so decoded frames are delivered via our
        // LockCallback/DisplayCallback in BGRA format. With HW accel enabled, VLC sends
        // video to the GPU directly and never calls our callbacks → audio plays, no video.
        _libVlc = new LibVLC(enableDebugLogs: false, "--avcodec-hw=none");
    ...
    public void UploadFrame(ID3D11DeviceContext ctx)
    {
        if (_needsNewTexture)
        {
            ...
            // Zero-initialize to black immediately after creation.
            // Dynamic textures are NOT guaranteed zero-filled — they contain whatever was in
            // that VRAM page before (often bright/white from previous scene data), which gets
            // amplified to pure white by FFXIV's bloom before any video frame arrives.
            if (_dynTex != null)
            {
                ctx.Map(_dynTex, 0, MapMode.WriteDiscard, Vortice.Direct3D11.MapFlags.None, out var zeroMap);
                unsafe
                {
                    long bytes = (long)zeroMap.RowPitch * _texHeight;
                    new System.Span<byte>((void*)zeroMap.DataPointer, (int)bytes).Clear();
                }
                ctx.Unmap(_dynTex, 0);
            }
        }
