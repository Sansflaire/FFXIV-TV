...
public sealed class SyncClient : IDisposable
{
    ...
    {
        // Normalise to ws:// / wss:// URI.
        // Addresses ending in .trycloudflare.com are cloudflared tunnel URLs — always WSS.
        string uri;
        if (address.StartsWith("ws://",  StringComparison.OrdinalIgnoreCase) ||
            address.StartsWith("wss://", StringComparison.OrdinalIgnoreCase))
            uri = address;
        else if (address.Contains(".trycloudflare.com", StringComparison.OrdinalIgnoreCase))
            uri = $"wss://{address.TrimStart('/')}/";
        else
            uri = $"ws://{address}/";
            ...
            try
            {
                ...
                switch (type)
                {
                    case "play":
                        ...
                        break;
                    case "state":
                        // Sent by host server on connect — brings client up to current state.
                        string stateUrl = msg["url"]?.Value<string>() ?? "";
                        float  statePos = msg["position"]?.Value<float>() ?? 0f;
                        bool   paused   = msg["isPaused"]?.Value<bool>() ?? false;
                        if (!string.IsNullOrEmpty(stateUrl))
                        {
                            OnPlay?.Invoke(stateUrl, statePos);
                            if (paused)
                                // Give VideoPlayer a moment to start before pausing.
                                Task.Delay(1200).ContinueWith(_ => OnPause?.Invoke());
                        }
                        break;
